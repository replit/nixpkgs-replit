...
describe('sourceUtils', () => {
    describe('wrapObjectLiteral', () => {
        ...
        for (const [input, output] of cases) {
            it(input, () => chai_1.expect(sourceUtils_1.wrapObjectLiteral(input)).to.equal(output));
        }
    });
    const wrapped = (...stmts) => [
        '(() => {',
    ...
    describe('logMessageToExpression', () => {
        const cases = {
            ...
            escaping: [
                ...
                `console.log("greet%%:o%%\\"' %O name:%%\\"'  %O %%", ${wrapped('return greet;')}, ${wrapped('return name;')})`,
            ],
            'complex expression': [
        ...
        for (const name of Object.keys(cases)) {
            ...
            it(name, async () => {
                const compiler = new logPoint_1.LogPointCompiler({
                    prepare: () => ({ canEvaluateDirectly: true }),
                });
                const compiled = compiler.compile({ line: 0 }, input).breakCondition;
                chai_1.expect(compiled.slice(0, compiled.lastIndexOf('\n'))).to.equal(expected);
            });
        }
    });
    describe('rewriteTopLevelAwait', () => {
        ...
        for (const [input, output] of tcases) {
            it(input, () => {
                const transformed = sourceUtils_1.rewriteTopLevelAwait(input);
                if (transformed === undefined || output === undefined) {
                    chai_1.expect(transformed).to.equal(output);
                }
                else {
                    asserts_1.assertAstEqual(transformed, output);
                }
            });
        }
    });
});
//# sourceMappingURL=sourceUtils.test.js.map
//# sourceMappingURL=sourceUtils.test.js.map
