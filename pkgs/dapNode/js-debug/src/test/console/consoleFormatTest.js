...
describe('console format', () => {
    testIntegrationUtils_1.itIntegrates('string', async ({ r }) => {
    ...
    testIntegrationUtils_1.itIntegrates('popular types', async ({ r }) => {
        const p = await r.launchAndLoad(`
        <p id="p"></p>
        <script>
          // Populate Globals
        ...
        await p.logger.evaluateAndLog([].concat(...expressions), { depth: 0 });
        p.assertLog();
    });
    testIntegrationUtils_1.itIntegrates('collections', async ({ r }) => {
        const p = await r.launchAndLoad(`
    ...
    testIntegrationUtils_1.itIntegrates('es6', async ({ r }) => {
        const p = await r.launchAndLoad(`
        <script>
    ...
    testIntegrationUtils_1.itIntegrates('es6-2', async ({ r }) => {
        const p = await r.launchAndLoad(`
        <script>
          var map2 = new Map();
          map2.set(41, 42);
    ...
    testIntegrationUtils_1.itIntegrates('array', async ({ r }) => {
        const p = await r.launchAndLoad(`
        <script>
          var a0 = [];
          var a1 = []; a1.length = 1;
    ...
    testIntegrationUtils_1.itIntegrates('class', async ({ r }) => {
        const p = await r.launchAndLoad(`
        <script>
          var a0 = [];
          var a1 = []; a1.length = 1;
    ...
    testIntegrationUtils_1.itIntegrates('groups', async ({ r }) => {
    ...
    testIntegrationUtils_1.itIntegrates('colors', async ({ r }) => {
        const p = await r.launchAndLoad(`blank`);
        await p.logger.evaluateAndLog([
    ...
    testIntegrationUtils_1.itIntegrates('error traces in source maps', async ({ r }) => {
    ...
    testIntegrationUtils_1.itIntegrates('applies skipfiles to logged stacks', async ({ r }) => {
        var _a;
      ...
      `, { skipFiles: ['**/ignore-me.js'] });
        ...
        await evaluation;
        handle.log(`logged ${output.output} at ${(_a = output.source) === null || _a === void 0 ? void 0 : _a.name}:${output.line}:${output.column}`);
        handle.assertLog();
    });
});
//# sourceMappingURL=consoleFormatTest.js.map
//# sourceMappingURL=consoleFormatTest.js.map
