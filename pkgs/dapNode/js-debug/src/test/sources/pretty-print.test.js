...
describe('pretty print sources', () => {
    async function waitAndStayPaused(p) {
        const { threadId } = p.log(await p.dap.once('stopped'));
        await p.logger.logStackTrace(threadId);
        return () => p.dap.continue({ threadId });
    }
    const testPrettyPrints = async (p, source) => {
    ...
    testIntegrationUtils_1.itIntegrates('base', async function ({ r }) {
    ...
    testIntegrationUtils_1.itIntegrates('steps in pretty', async ({ r }) => {
        const p = await r.launchUrl('pretty/pretty.html');
        const source = { path: p.workspacePath('web/pretty/ugly.js') };
        await p.dap.setBreakpoints({ source, breakpoints: [{ line: 5, column: 1 }] });
        p.load();
    ...
    testIntegrationUtils_1.itIntegrates('bps', async ({ r }) => {
        const p = await r.launchUrl('pretty/pretty.html');
        const source = { path: p.workspacePath('web/pretty/ugly.js') };
        ...
        p.log(await p.dap.setBreakpoints({
    ...
    testIntegrationUtils_1.itIntegrates('eval sources (#929)', async function ({ r }) {
...
//# sourceMappingURL=pretty-print.test.js.map
//# sourceMappingURL=pretty-print.test.js.map
