...
describe('completion', () => {
    ...
    testIntegrationUtils_1.itIntegrates('completion', async ({ r }) => {
    ...
    `);
        for (const [completion, expected] of tcases) {
            ...
            chai_1.expect(actual.targets.slice(0, 3)).to.deep.equal(expected, `bad result evaluating ${completion}`);
        }
    });
    testIntegrationUtils_1.itIntegrates('$returnValue', async ({ r }) => {
        const getFrameId = async () => (await p.dap.stackTrace({
            threadId: threadId,
        })).stackFrames[0].id;
        const p = await r.launchAndLoad(`
      <script>
    ...
    `);
        ...
        chai_1.expect(a1.targets).to.not.containSubset([
            {
        ...
        chai_1.expect(a2.targets).to.containSubset([
            {
                label: '$returnValue',
                type: 'variable',
                sortText: '~$returnValue',
        ...
        chai_1.expect(a3.targets).to.containSubset([
            {
                label: 'a',
                sortText: '~~a',
                type: 'property',
